...
const SidebarNav = React.memo(function SidebarNav() {
  ...
  const openWhatTaskTodayPage = useAppStore((s) => s.openWhatTaskTodayPage)
  ...
  const whatTaskTodayActive = activeView === 'what-task-today'
  ...
  return (
    ...
    >
      ...
      <button
        type="button"
        onClick={openWhatTaskTodayPage}
        aria-current={whatTaskTodayActive ? 'page' : undefined}
        className={cn(
          'flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-[13px] font-medium tracking-tight transition-colors',
          whatTaskTodayActive
            ? 'bg-worktree-sidebar-accent text-worktree-sidebar-accent-foreground'
      ...
      >
        <ClipboardList
          className={cn(
            'size-4 shrink-0',
            !whatTaskTodayActive && 'text-worktree-sidebar-foreground/30'
          )}
          strokeWidth={whatTaskTodayActive ? 2.25 : 1.75}
        />
        <span className="flex-1">
          {translate('auto.components.sidebar.SidebarNav.whatTaskToday', 'What Task Today')}
        </span>
      </button>
